...
from main_compare2 import linear_beta_schedule
import numpy as np
# =========================================================
...
def find_latest_ckpt(folder):
    ...
    if best_file is None:
        print(f"❌ No valid ckpt in {folder}")
        return None
...
def load_all_models(model_class, root="./models", device="cuda"):
    ...
    for folder in os.listdir(root):
        ...
        # 🔥 keep full folder name (avoid overwrite)
        key = folder
...
'''
@torch.no_grad()
def ddim_sample(model, device, shape=(64,1,28,28), steps=500, eta=0.0):
...
def group_by_C(models):
    ...
    for name, model in models.items():
        ...
        match = re.search(r"(M\d+)_C(\d+)", name)
        if not match:
            continue
        ...
        # 🔥 keep full name (no overwrite)
        grouped[C][name] = model
...
def compare_sampling_steps(model_dict, device, C, steps_list=[10,20,50,100]):
    ...
    for name, model in model_dict.items():
        ...
        for steps in steps_list:
            ...
            save_path = f"samples/{name}_steps{steps}.png"

            vutils.save_image(samples, save_path, nrow=8)

            print(f"✅ {save_path}")

            #save_path2 = f"samples/{name}_steps{steps}_channels_t500.png"

            #visualize_channels(model, device, save_path2, t_value=200, sample_idx=0)

            #save_path3 = f"samples/{name}_steps{steps}_channels_t100.png"
...
def run(model_class, device):
    ...
    for C, model_dict in grouped.items():
        ...
        print(f"\n🔥 Compare C={C}")
        print("Models:", list(model_dict.keys()))

        compare_sampling_steps(
            model_dict,
            device,
            C,
            steps_list=[100,200,500,800, 1000]   # safer default
        )
...
if __name__ == "__main__":
    ...
    run(UNet, device)
    model_dict = load_all_models(UNet)
...
if __name__ == "__main__":
    ...
    # 🔥 load once
    ROOT = os.path.dirname(os.path.abspath(__file__))
    MODEL_DIR = os.path.join(ROOT, "models_old")
    ...
    for C, model_dict in grouped.items():
        ...
        compare_sampling_steps(
            ...
            device,
            C,
            steps_list=[10]#,20, 50, 100,200,500,800,1000]   # ✅ better
        )
    ...
    for name, model in models.items():
        print(f"\nEvaluate: {name}")
        evaluate_model(model,device=device)   # add device if needed